    ...
    <table class="table-auto border-2 border-border-gray mx-auto">
        <thead>
            <tr>
                ...
                <th class="border-2 border-border-gray  bg-gray-600  text-letter text-sm   mr-2 w-5/12">
                    Description
                </th>
                <th class="border-2 border-border-gray bg-gray-600  text-letter text-sm   mr-2 w-40 ">
                    Issued to
            ...
            @foreach ($inventories as $inventory)
                <tr class="bg-white border-b transition duration-300 ease-in-out hover:bg-gray-100">
                    ...
                    <th scope="row"
                        ...
                        {{ $inventories->firstItem() + $loop->index }}.</th>
                    <td class='border-2 border-border-gray text-grey text-left text-sm '>
                        {{ $inventory->property_number }}</td>
                    <td class='border-2 border-border-gray text-grey text-center text-sm '>{{ $inventory->quantity }}
                    </td>
                    <td class='border-2 border-border-gray text-grey text-left text-sm '>{{ $inventory->property_name }}
                    </td>
                    ...
                    <td class='border-2 border-border-gray  text-grey text-center text-sm '>
                        {{ App\Models\User::find($inventory->user_id)->name }}
                    <td class='border-2 border-border-gray  text-grey text-center text-sm '>
                        <div class="flex flex-row justify-center p-1">
                            ...
                            <a href="{{ url('propertyupdate/' . $inventory->id) }}"
