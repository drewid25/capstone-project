...
class CompanyController extends Controller
{
  ...
  public function store(Request $request)
  {
    ...
    $newCompanyImage = time() . '-' . $request->company_name . "."  . $request->company_logo->extension();
    $request->company_logo->move(public_path('images/company/'), $newCompanyImage);
    $company = Company::create([
      'company_name' => $request->input('company_name'),
      'company_add' => $request->input('company_add'),
      'email' => $request->input('email'),
      'company_logo' => $newCompanyImage,
