<div class="bg-gray-300 py-6 px-3 h-full col-span-10 border-r-2 border-l-2 border-r-letter overflow-auto">
    ...
    <table class="container border-2 border-border-gray ">
        <thead>
            <tr>
                <th class="border-2 border-border-gray bg-gray-600 text-letter text-sm mr-2 w-16">
                    No.
                </th>
        ...
        <tbody>
            ...
            @foreach ($users as $employee)
                <tr class="bg-white border-b transition duration-300 ease-in-out hover:bg-gray-100">
                    <th scope="row"
                        class='flex items-center p-3 border-2 border-border-gray text-gray-800 font-medium capitalize text-left text-sm'>
                        {{ $i++ }}.
                        <img class="h-10 w-10  ml-2" src="{{ asset('images/users/' . $employee->employee_image) }}"
                            alt="">
                    </th>
                    <td class='border-2 border-border-gray text-gray-800 font-medium capitalize text-left text-sm'>
                        {{ $employee->id_number }}
                    </td>
                    <td class='border-2 border-border-gray text-gray-800 font-medium  capitalize text-left text-sm'>
                        {{ $employee->name }}</td>
                    ...
                    <td class='border-2 border-border-gray text-gray-800 font-medium  apitalize text-left text-sm'>
                        {{ $employee->email }}</td>
    ...
    <div class="text-center mt-10">
        ...
        <a href="/addemployee"
            class="text-lg font-bold hover:bg-gray-100 bg-gray-800 text-gray-100 p-2 hover:text-gray-800 hover:border hover:border-gray-600 rounded"><i
                class="fa-solid fa-address-book   mr-2 "></i>ADD
            NEW EMPLOYEE</a>
                ...
                success: function(data) {
                    ...
                    data.forEach(function(property) {
                        $('#subjectarea').append(
                            ...
                            '</td><td class="border-2 border-border-gray text-gray-800 font-medium  capitalize text-left text-sm" >' +
