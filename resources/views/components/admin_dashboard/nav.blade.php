                            ...
                            @foreach ($companies as $company)
                                <a href="/dashboard">
                                    <div class="flex  items-center ml-5">
                                        <img class="h-14  w-14 rounded-full mr-3"
                                            src="{{ asset('images/company/' . $company->company_logo) }}" alt="">


                                        <div class="flex flex-col ">
                                            <h2 class="text-xl font-bold text-gray-800">{{ $company->company_name }}
                                            </h2>
                ...
                <div class="hidden md:block">
                    ...
                    <div class=" ml-3 flex">
                        <img class="h-8 w-8 rounded-full"
                            src="{{ asset('images/users/' . auth()->user()->employee_image) }}" alt="">

                        <h4 class="text-gray-800 ml-3 capitalize items-center">{{ auth()->user()->name }}</h4>
                        <form action="/logout" method="post">
                            @csrf
                            <button
                                class="text-gray-100 ml-2  hover:bg-gray-100 hover:text-gray-800  bg-gray-800 px-3 py-2 rounded-md text-sm font-medium">logout</button>
                        </form>
                    </div>
                </div>
            </div>
